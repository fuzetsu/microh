...
const input = fss.readFileSync('microh.js', 'utf8')
...
const moduleToBrowser = code =>
  umdBoiler(
    code
      ...
      .replace(/export\s+default\s+([^\s]+)/gi, 'exports = $1') // since no named exports for now don't depend on object.assign
      .replace(/^/gm, '  ')
  )
